...
(defn- schedule-event-log
  "Schedule `github-api-client.event-log/pull-requests-logger` to run every `interval-ms` milliseconds, pulling the last
  `last` pull requests from GitHub repository `org`/`repo`, storing them in our database. Use configuration hash `config`
  to configure GitHub API URL, access token and so forth.
  ...
  The purpose of `stop-chan` is for the main thread to block on it so that the application does not exit immediately."
...
(defn make-scheduler
  "Take an `atom` `schedules` wrapping a `hash` and return a function that takes `interval-ms`, `org`, `repo`, `last`
  as arguments.

  When called, the returned function will schedule `github-api-client.event-log/pull-request-logger` to run every
  `interval-ms` against the GitHub repository `org/repo`, on each run pulling the last `last` pull requests, storing
  them in our permanent storage. 
  ...
  Store `stop-fn` and `stop-chan` under key \"`org/repo`\" in `schedules`, our state holder."
  [schedules db config]
...
(defn cancel-schedule
  "Take organization `org`, repository `repo` and the state `atom` `schedules` containing all scheduled tasks. Look up
  any (unique) import task associated with `org`/`repo`. If it exists cancel it and return `true`. Otherwise do nothing
  and return `false`."
  [org repo schedules]
  (let [key (str org "/" repo)]
    (if-let [{:keys [stop-fn]} (get @schedules key)]
      (do
        (stop-fn)
        (swap! schedules #(dissoc % key))
        true)
      false)))
